  ...
  <form method="POST" @submit.prevent="onSubmit" @keydown="grupo.errors.clear($event.target.name)">
    ...
    <div class="row">
      <div class="col-4 offset-4">
        ...
        <b-card >
          <input-texto titulo="Grupo de produtos" disabled=true v-model="nome" />
          <input-texto titulo="Nome do sub-grupo" v-model="tipo.nome" />
        </b-card>
...
<script>
  ...
  import VueSticky from 'vue-sticky';
    ...
    export default {
      ...
      beforeMount(){
        if (this.valor.id != ""){
          this.tipo.id = this.valor.id;
          this.tipo.nome = this.valor.nome;
        }
      ...
      methods: {
        onSubmit() {
          ...
          this.tipo.post(base_url + 'novo/produtos/grupos/' + this.id_selecionado + '/tipos')
            .then(function(response){
              self.$emit("salvo")

              self.$root.$refs.toastr.s("Sub-grupo criado com sucesso", "Informativo");
            });
        },
        voltar(){
          this.$emit('voltar');
